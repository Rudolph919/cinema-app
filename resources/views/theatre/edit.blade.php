<x-app-layout>
    ...
    <div class="max-w-5xl bg-white shadow-2xl rounded-lg mx-auto py-4 mt-10">
        <div class="p-6">
            <form method="POST" action="{{ route('theatre.update', $theatre) }}">
                @csrf
                @method('PUT')
                <div class="">
                    <div class="space-y-6">
                        ...
                        <div class="">
                            ...
                            <div class="mt-1">
                                <input type="text" name="theatre_name" id="theatre_name"
                                       autocomplete="theatre_name" value="{{ $theatre->name }}"
                            ...
                            <div class="mt-1">
                                ...
                                <select name="theatre_cinema" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    @foreach($cinemas as $cinema)
                                        <option value="{{ $cinema->id }}" {{ $cinema->id == $theatre->cinema_id ?
                                        'selected' : '' }}>{{ $cinema->name }}</option>
                                    @endforeach
                                </select>
